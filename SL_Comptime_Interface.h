...
// Template function definitions. Implementation in .cpp file when possible 
...
namespace CMF {
	...
	template <typename T>									T min(T& value, T& value2) {	//  Maximum value between two variables
		if (value > value2) {
			return value2;
		}
		return value;
	}
	template <typename T, int cols>						    T min(T(&arr_data)[cols]) {

		T minval = arr_data[0];	// Set current max value as first element

		for (int i = 1; i < cols; i++) {	// Start at 1 as first element is currently max value
			if (arr_data[i] < minval) { minval = arr_data[i]; }
		}
		return minval;
	}
	template <typename T, int rows, int cols>				T min(T(&arr_data)[rows][cols]) {

		T minval = arr_data[0][0];	// Set current max value as first element

		for (int i = 0; i < rows; i++) {	// Start at 1 as first element is currently max value
			for (int j = 0; i < cols; j++) {
				if (arr_data[i][j] < minval) { minval = arr_data[i][j]; }
			}
		}
		return minval;
	}
	template <typename T, int rows, int cols, int depth>	T min(T(&arr_data)[depth][rows][cols]) {

		T minval = arr_data[0][0][0];	// Set current max value as first element

		for (int i = 0; i < depth; i++) {
			for (int j = 0; j < rows; j++) {
				for (int k = 0; k < cols; k++) {
					if (arr_data[i][j][k] < minval) { minval = arr_data[i][j][k]; }
				}
			}
		}
		return minval;
	...
	template <typename T, int cols, int cols_>				void concatenate(T(&arr_data)[cols], T(&arr_data2)[cols_], T(&concatted_arr)[cols + cols_]) {
		...
		//	concatted_arr[j2 + cols] = arr_data2[j2];
		//}
	}
